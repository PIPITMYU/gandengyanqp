...
//		interfaceIds.put(100202, 1);
		...
		//游戏端代开历史造数据
		interfaceIds.put(100400, 1);
		//游戏端玩家战绩造数据
		interfaceIds.put(100401, 1);
		//俱乐部玩家战绩造数据
		interfaceIds.put(100402, 1);
		//俱乐部
		...
		interfaceIds.put(999800, 1);
		//获取字段解析
		interfaceIds.put(100999, 0);
		
		interfaceIds.put(999997, 1);
		interfaceIds.put(999998, 1);
		interfaceIds.put(999999, 1);
	}
	...
	static class ReceiveTask implements Runnable{
		...
		public void run() {
			...
			try {
				log.info("["+interfaceId+"-RECEIVE_DATE]:"+readData);
...
//				wsClient.getChannelHandlerContext().close();
	...
	private static void logic(WSClient wsClient,Map<String,Object> readData,int interfaceId) throws Exception{
		...
		case 100201:
			...
			break;
//		case 100202://铺分接口,不要了
//			GameFunctions.interface_100202(wsClient, readData);
//			break;
		case 100203:
		...
		//玩家造数据
		case 100400:
			RoomUtil.interface_100400(wsClient, readData);
			break;
		case 100401:
			RoomUtil.interface_100401(wsClient, readData);
			break;
		case 100402:
			RoomUtil.interface_100402(wsClient, readData);
			break;
		...
		case 100999:
			...
			break;
		case 999800:
			MessageFunctions.interface_999800(wsClient, readData);
			break;
