...
public class RedisUtil {
	public static final Log log = LogFactory.getLog(MyRedis.class);
	...
	public static void hmset(String key, Map<String, String> info,
			Integer timeout) {
		Jedis jedis = null;
		try {
			jedis = MyRedis.getRedisClient().getJedis();
	 ...
	 * @param player
	 */
	public synchronized static boolean updateRedisData(RoomResp room,
			Player player,String cid) {
		boolean result = true;
		if (room != null) {
		...
		if (room != null) {
			...
			try {
				...
				for (int i = 0; i < userIds.length; i++) {
					if (userIds[i] != null) {
						Player p = (Player) JSON.parseObject(jedis.get(Cnst.get_REDIS_PREFIX_USER_ID_USER_MAP(cid).concat(String.valueOf(userIds[i]))),
										Player.class);
						if (p != null) {
							players.add(p);
						}
	...
	public synchronized static Player getPlayerByUserId(String userId,String cid) {
		...
		try {
			jedis = MyRedis.getRedisClient().getJedis();
			player = (Player) JSON.parseObject(jedis
					.get(Cnst.get_REDIS_PREFIX_USER_ID_USER_MAP(cid).concat(userId)),
					Player.class);
		} catch (Exception e) {
			log.error(e);
			MyRedis.getRedisClient().returnBrokenJedis(jedis);
	...
	public synchronized static Long getUserIdByOpenId(String openId,String cid) {
		...
		try {
			jedis = MyRedis.getRedisClient().getJedis();
			String string = jedis.get(Cnst.get_REDIS_PREFIX_OPENIDUSERMAP(cid)
					.concat(openId));
			if (string != null) {
		...
		try {
			if (players != null && players.size() > 0) {
				for (Player p : players) {
					if(p == null)
						continue;
					jedis.set(Cnst.get_REDIS_PREFIX_USER_ID_USER_MAP(cid).concat(String
							.valueOf(p.getUserId())), JSON.toJSONString(p));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	public synchronized static boolean setRoomRespByRoomId(String roomId,
			RoomResp room,String cid) {
		boolean result = true;
		Jedis jedis = null;
		try {
			jedis = MyRedis.getRedisClient().getJedis();
			jedis.set(Cnst.get_REDIS_PREFIX_ROOMMAP(cid).concat(roomId),
					JSON.toJSONString(room,SerializerFeature.DisableCircularReferenceDetect));
		} catch (Exception e) {
			result = false;
			MyRedis.getRedisClient().returnBrokenJedis(jedis);
		} finally {
	...
	public synchronized static boolean setPlayerByUserId(Player player,String cid) {
		...
		try {
			jedis = MyRedis.getRedisClient().getJedis();
			jedis.set(Cnst.get_REDIS_PREFIX_USER_ID_USER_MAP(cid).concat(String
					.valueOf(player.getUserId())), JSON.toJSONString(player));
		} catch (Exception e) {
			result = false;
			MyRedis.getRedisClient().returnBrokenJedis(jedis);
		} finally {
	 ...
	 * @return
	 */
	public synchronized static boolean setUserIdByOpenId(String openId,
			String userId,String cid) {
		boolean result = true;
		Jedis jedis = null;
		try {
			jedis = MyRedis.getRedisClient().getJedis();
	 ...
	 */
	public synchronized static ClubInfo getClubInfoByClubId(String key) {
		ClubInfo info = null;
		Jedis jedis = null;
		try {
			jedis = MyRedis.getRedisClient().getJedis();
	 ...
	 * @param string
	 */
	public static Integer getTodayJuNum(String key,String cid) {
		Integer result =0;
		Jedis jedis=null;
		try {
			jedis = MyRedis.getRedisClient().getJedis();
			result=Integer.valueOf(jedis.get(Cnst.get_REDIS_CLUB_TODAYKAI_NUM(cid).concat(key)));
		} catch (Exception e) {
			result=null;
			MyRedis.getRedisClient().returnBrokenJedis(jedis);
		} finally {
	 ...
	 * @param intValue
	 */
	public  synchronized static boolean setTodayJuNum(String key, Integer num, Integer timeout,String cid) {
		boolean result = true;
		Jedis jedis = null;
		try {
			jedis = MyRedis.getRedisClient().getJedis();
			jedis.set(Cnst.get_REDIS_CLUB_TODAYKAI_NUM(cid).concat(key),num+"");
			if (timeout != null) {
				jedis.expire(key, timeout);
			}
		} catch (Exception e) {
