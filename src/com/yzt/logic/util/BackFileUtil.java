...
public class BackFileUtil {
	...
	public static void save(Integer interfaceId, RoomResp room, List<Player> players, Object infos, Action action,String cid) {
		// 俱乐部房间 现在也要回放
//		if (String.valueOf(room.getRoomId()).length() > 6) {
//			return;
//		}
		Integer xjn = room.getXiaoJuNum() == null ? 1 : room.getXiaoJuNum();
		String redisKey = Cnst.REDIS_HUIFANG_MAP.concat(cid+"_"+room.getRoomId() + "_" + room.getCreateTime() + "_" + xjn);
		...
		case 100200:
			if (room.getState() == Cnst.ROOM_STATE_GAMIING ) {//房间状态只能为游戏中
				...
				for (Player p : players) {
					...
					users.put("pais", p.getCurrentPaiList());
					userInfo.add(users);
				}
				map.put("interfaceId", interfaceId);
				info.put("userInfo", userInfo);
				Map<String, Object> roomInfo = MessageFunctions.getRoomInfo(room,null);
				roomInfo.put("nextActionUserId", room.getNextActionUserId());
				roomInfo.put("nextAction", room.getNextAction());
		...
		case 100201:
			...
			if(action.getType() == Cnst.ACTION_TYPE_FAPAI){
				actions.put("extra", action.getExtra().get(0));
			}else{
				actions.put("extra", action.getExtra());
			}
			
			actions.put("userId", action.getUserId());
			actions.put("nextAction", room.getNextAction());
			if(room.getState() != Cnst.ROOM_STATE_XJS){
				actions.put("nextActionUserId", room.getNextActionUserId());
			}			
